---
layout: archive
title: ""
permalink: /onsparetime/
author_profile: true
redirect_from:
  - /resume
---


{% include base_path %}


## Presentations
----
  - [Automated Speech Recognization in English] 22/10/2024  



## Projects

{% include base_path %}
----
{% for post in site.aiprojects %}
  {% include archive-single.html %}
{% endfor %}


[Automated Speech Recognization in English]: ../Documents/ASR-Presentation.pdf



## Certifications/Courses
----
  - [IIIT-H] : PG Certification Program in Artificial Intelligence and Machine Learning
  - [AWS] : AWS Cloud Practitioner
  - [Microsoft] : Microsoft Certified: Azure Fundamentals
  - [AWS] : AWS Certified Security – Specialty
  - [sAFE] : Certified SAFe® 6 Agilist
  - [Scrum Alliance] : Certified ScrumMaster
  - [Linux] : Green Software for Practitioners
  - [DevOps Institute] : Site Reliability Engineering (SRE) Foundation℠ Certification
  - [Udemy] : REST API Management, Monitoring & Analytics using Kong 3
  - [Udemy] : Microservices Software Architecture: Patterns and Techniques
  - [Linkedin Learning] : Design a Cloud Migration Strategy
  - [Linkedin Learning] : Microservices: Security
  - [Linkedin Learning] : Cloud Security Architecture for the Enterprise (2022)
  - [Linkedin Learning] : Mentoring Others
  - [Linkedin Learning] : Leadership Foundations (2013)

